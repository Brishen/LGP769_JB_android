...
int ecryptfs_clear_keyring(void)
{
	...
	rc = (int) keyctl_clear(KEY_SPEC_USER_KEYRING);
        if (rc < 0) {
                rc = errno;
                syslog(LOG_ERR, "Failed to clear key rc=%d\n", rc);
                goto out;
        }
	return 0;
out:
...
#if 0 // hyunk
		rc = 0;
		syslog(LOG_ERR, "%s:: hyunk already in keyring\n", __FUNCTION__);
#else
		rc = 1;
#endif
		goto out;
...
#endif /* #ifdef ENABLE_NSS */
{
	...
	//imsi = unlink(filename);
	syslog(LOG_ERR, "%s:%d:: unlink file=%s ret=%d errno=%d encrypted_passphrase_bytes=%d\n",
		       __FUNCTION__, __LINE__, filename, imsi, errno, encrypted_passphrase_bytes);
    for (i = 0; i < 10; i++)
    {
	if ((fd = open(filename, (O_WRONLY | O_CREAT | O_TRUNC | O_SYNC /*| O_EXCL*/),
		       (S_IRUSR | S_IWUSR))) == -1) {
		syslog(LOG_ERR, "Error attempting to open [%s] for writing err=%d\n",
		       filename, errno);
		rc = -EIO;
		goto out;
	}
	if ((size = write(fd, wrapping_auth_tok_sig,
		...
		syslog(LOG_ERR, "Error attempting to write encrypted "
		       "passphrase ([%d] bytes) to file [%s]; size = [%zu]; err=%d\n",
		       encrypted_passphrase_bytes, filename, size, errno);
		rc = -EIO;
		close(fd);
		goto out;
	}
	...
	close(fd);
        ...
        if (rc == -1)
        {
	    syslog(LOG_ERR, "[%d]::Error Stat wrapping file err=%d\n", i, errno);
            continue;
        }
        if (file_info.st_size == 0)
        {
	    syslog(LOG_ERR, "[%d]::Error file Size 0!!!\n", i);
            continue;
        }
	syslog(LOG_ERR, "[%d]::Success writing wrapping file !!!\n", i);
        break;
    }
}
	rc = 0;
