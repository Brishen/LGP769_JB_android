...
int ecryptfs_register_key_modules(struct ecryptfs_ctx* ctx)
{
	...
	printf("%s, %d dir_name=%s\n", __FUNCTION__, __LINE__, (dir_name != NULL)? dir_name:"null"); 
...
//	while ((ep = readdir(dp))) 
	...
	//		continue;
		...
		if (asprintf(&path, "%s/libecryptfs_key_mod_passphrase.so", key_mod_dir) == -1) {
	...
	printf("%s, %d handle=0x%x\n", __FUNCTION__, __LINE__, handle); 
		if (!handle) {
// hyunk			syslog(LOG_ERR, "Could not open library handle\n");
			free(path);
			goto out; // hyunk end_loop;
		}
	printf("%s, %d\n", __FUNCTION__, __LINE__); 
		get_key_mod_ops = (struct ecryptfs_key_mod_ops *(*)(void))
			dlsym(handle, "get_key_mod_ops");
	printf("%s, %d get_key_mod_ops=0x%x\n", __FUNCTION__, __LINE__, get_key_mod_ops); 
		if (!get_key_mod_ops) {
		...
		{
			syslog (LOG_ERR, "hyunk Succeeded to get the symbol "
				"[get_key_mod_ops] from key module [%s]\n ",
				path);
		}
