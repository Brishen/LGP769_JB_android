...
void Document::updateStyleIfNeeded()
{
    ...
    // CAPPFIX_WEB_INITIALIZE_FULLSCREEN_ELEMENT
    if (webkitIsFullScreen())
        return;
    // CAPPFIX_WEB_INITIALIZE_FULLSCREEN_ELEMENT_END
...
{
    // CAPPFIX_WEB_ADD_SEMICOLON_IN_SEPARATOR : WFT-1374
    return c == ' ' || c == '\t' || c == '\n' || c == '\r' || c == '=' || c == ',' || c == '\0' || c == ';';
}
...
{
// CAPPFIX_WEB_FULLSCREEN_VIDEO: fix bug : play button works only one time.
// CAPPFIX_WEB_VIDEO_DETACH_BUG_FIX : if 0 to if 1.
#if 1
    ...
    if (m_fullScreenElement) {
        webkitDidExitFullScreenForElement(0);
    } else if (page()->mainFrame()->document()->webkitIsFullScreen()) {
        page()->mainFrame()->document()->webkitDidExitFullScreenForElement(0);
    }
    // m_fullScreenElement = 0; // original code
    // CAPPFIX_WEB_INITIALIZE_FULLSCREEN_ELEMENT_END
#endif
}
