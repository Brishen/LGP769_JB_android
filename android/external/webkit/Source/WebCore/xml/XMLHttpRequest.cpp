...
static bool isValidHTTPHeaderValue(const String& name)
{
...
Blob* XMLHttpRequest::responseBlob(ExceptionCode& ec)
{
    ...
    if (!m_responseBlob.get()) {
        ...
        if (m_binaryResponseBuilder.get() && m_binaryResponseBuilder->size() > 0) {
            ...
            rawData->mutableData()->append(m_binaryResponseBuilder->data(), m_binaryResponseBuilder->size());
            ...
            m_responseBlob = Blob::create(blobData.release(), m_binaryResponseBuilder->size());
            m_binaryResponseBuilder.clear();
        } else {
            // If we errored out or got no data, we still return a blob, just an empty one.
            m_responseBlob = Blob::create(blobData.release(), 0);
        }
    }
...
void XMLHttpRequest::setResponseType(const String& responseType, ExceptionCode& ec)
{
    ...
    if (!m_async && scriptExecutionContext()->isDocument() && m_url.protocolInHTTPFamily()) {
...
void XMLHttpRequest::open(const String& method, const KURL& url, bool async, ExceptionCode& ec)
{
    ...
    // We'll only disable this functionality for HTTP(S) requests since sync requests for local protocols
    // such as file: and data: still make sense to allow.
    if (!async && scriptExecutionContext()->isDocument() && url.protocolInHTTPFamily() && m_responseTypeCode != ResponseTypeDefault) {
...
void XMLHttpRequest::setRequestHeader(const AtomicString& name, const String& value, ExceptionCode& ec)
{
    ...
    if (!scriptExecutionContext()->securityOrigin()->canLoadLocalResources() && !isAllowedHTTPHeader(name)) {
