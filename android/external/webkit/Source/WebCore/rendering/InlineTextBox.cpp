...
void InlineTextBox::paint(PaintInfo& paintInfo, int tx, int ty, int /*lineTop*/, int /*lineBottom*/)
{
    ...
    // CAPPFIX_WEB_RTL_ELLIPSIS
    bool shouldCreateRunWithLength = false;
    // CAPPFIX_WEB_RTL_ELLIPSIS_END
    ...
    if (m_truncation != cNoTruncation) {
        ...
        if (renderer()->containingBlock()->style()->isLeftToRightDirection() != isLeftToRightDirection()) {
            ...
            if (isHorizontal()) {
                int txTemp = tx;
                txTemp += isLeftToRightDirection() ? widthOfHiddenText : -widthOfHiddenText;
                // If the value of txTemp becomes less than tx (for RTL cases), then don't override tx,
                // otherwise its text will overlap on the left one's text.
                if (!isLeftToRightDirection() && txTemp < tx)
                    shouldCreateRunWithLength = true;
                else
                    tx = txTemp;
            } else {
            ...
            int widthOfVisibleText = toRenderText(renderer())->width(m_start, m_truncation, textPos(), m_firstLine);
            int widthOfHiddenText = m_logicalWidth - widthOfVisibleText;
            // For RTL text contained in an RTL container the ellipsis appear at the left edge of its container.
            // So, make the visible fragment of text hug the right edge of its container,
            // so that it does not overlap the ellipsis.
            tx += widthOfHiddenText;
            shouldCreateRunWithLength = true;
        }
        // CAPPFIX_WEB_RTL_ELLIPSIS_END
    }
    ...
    // CAPPFIX_WEB_RTL_ELLIPSIS
    // Modified the length here, so that textRun is created with the modified value of length.
    if (shouldCreateRunWithLength && m_truncation != cNoTruncation)
        length = m_truncation;
    // CAPPFIX_WEB_RTL_ELLIPSIS_END
...
void InlineTextBox::paintTextMatchMarker(GraphicsContext* pt, const FloatPoint& boxOrigin, const DocumentMarker& marker, RenderStyle* style, const Font& font)
{
    ...
    int deltaY = renderer()->style()->isFlippedLinesWritingMode() ? selectionBottom() - logicalBottom() : logicalTop() - selectionTop();
    int selHeight = selectionHeight() - deltaY;  // CAPPFIX_WEB_MARKER_INVALID_DRAW [WFT_2111]
    // CAPPFIX_WEB_PROPER_TYPE_CAST
    int sPos = max<int>(marker.startOffset - m_start, 0);
    int ePos = min<int>(marker.endOffset - m_start, m_len);
    // CAPPFIX_WEB_PROPER_TYPE_CAST_END
    TextRun run(textRenderer()->text()->characters() + m_start, m_len, textRenderer()->allowTabs(), textPos(), m_expansion, expansionBehavior(), !isLeftToRightDirection(), m_dirOverride || style->visuallyOrdered());
...
#if PLATFORM(ANDROID)
        // CAPPFIX_WEB_MARKER_INVALID_DRAW [WFT_2111]
        pt->drawHighlightForText(font, run, FloatPoint(boxOrigin.x(), boxOrigin.y()), selHeight, color, style->colorSpace(), sPos, ePos, marker.activeMatch);
#else
