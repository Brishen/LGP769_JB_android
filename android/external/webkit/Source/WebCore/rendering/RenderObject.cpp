...
void RenderObject::updateHitTestResult(HitTestResult& result, const IntPoint& point)
{
    ...
    if(result.isRectBasedTest())
    {
        ...
        if (isText())
            rect = toRenderText(this)->linesBoundingBox();
        else if (isRenderInline())
            rect = toRenderInline(this)->linesVisualOverflowBoundingBox();
        else if (isBox())
            rect = toRenderBox(this)->visualOverflowRect();

        const IntRect resultRect = result.rectForPoint(point.x(), point.y());
        rect.intersect(resultRect);
        intersectSize = rect.width() * rect.height();

        /* need to choice first node if sizes of intersect are same */
        if(result.m_maxIntersectSize >= intersectSize)
            return;
    }
    else
    // CAPPFIX_WEB_RECT_BASE_HITTEST_END
        if (result.innerNode())
            return;
    ...
    if (n) {
        ...
        if(result.isRectBasedTest()) result.m_maxIntersectSize = intersectSize; // CAPPFIX_WEB_RECT_BASE_HITTEST
    }
}
