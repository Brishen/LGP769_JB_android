...
void HTMLInputElement::updateType()
{
    ...
    if (m_inputType->isDateTimeField()) {
        if (m_inputType->isDateTimeType())
            m_data.setSize(25); // 25 is length of datetime string in format YYYY-MM-DDThh:mm:ssTZD
        else
            m_data.setSize(10); // 10 is length of date string in format YYYY-MM-DD
    }
...
void HTMLInputElement::parseMappedAttribute(Attribute* attr)
{
    if (attr->name() == nameAttr) {
        ...
        setNeedsValidityCheck();
    // CAPPFIX_WEB_HTML5: Form - disabled attribute for fieldset element
