...
//+++ BRCM
#ifdef BT_ALT_STACK
	// BTLA blz20_wrapper_connect() does a swap.
	// bdaddr_t in BTLA also is swapped.
	// So we need to do a swap here to undo one swap, so that the
	// Stack sees the bdaddr with b[0] being the MSB...
	baswap(&addr.l2_bdaddr,dst);
#endif
//--- BRCM
...
//+++ BRCM
#ifdef BT_ALT_STACK
		// Bind not suppported for client connections in BTLA
