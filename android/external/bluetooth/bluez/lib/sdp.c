...
int utf8_validate(const char *str, int start, int len)
{
	...
	while (p < end) {
		...
		/* Special-case ASCII; this makes us go a lot faster in D-Bus
		 * profiles where we are typically validating function names
		 * and such. We have to know that all following checks will
		 * pass for ASCII though, comments follow ...
		 */
		if (*p < 128) {
			++p;
			continue;
	...
	/* See that we covered the entire length if a length was passed in */
	return p == end;
}

static inline void copy_str(char* dest, const char* src, int len)
{
	int bValidUtf8 = utf8_validate(src, 0, len);
	int i;
	for (i = 0; i < len; i++) {
