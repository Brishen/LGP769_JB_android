...
//+++ BRCM
	const gchar *path;
	gboolean temporary_key = device_is_temporary(device);
...
*/
	dbus_message_append_args(req->msg,
				...
				DBUS_TYPE_STRING, &uuid,
				DBUS_TYPE_BOOLEAN, &temporary_key,
				DBUS_TYPE_INVALID);
//--- BRCM
...
*/
	err = agent_call_authorize(req, device, uuid);
//--- BRCM
	if (err < 0) {
...
*/
	DBG("authorize request was sent for %s", device_get_path(device));
//--- BRCM
