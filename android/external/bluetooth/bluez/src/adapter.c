...
//+++ BRCM
#ifdef BT_ALT_STACK
extern uint32_t dtun_add_sdp_record(DBusMessage *msg);
extern void     dtun_del_sdp_record(uint32_t handle);
extern void     dtun_add_ble_device_do_append(char *key, char *value, uint8_t *device_type, gboolean *is_paired_device);
extern void     dtun_add_devices_do_append(char *key, char *value);
#endif
//--- BRCM
					...
					guint interval)
{
	...
	if (adapter->pairable && interval <= 60)
		...
		adapter_set_limited_discoverable(adapter, FALSE);
//+++ BRCM
	/* glib timeout is not working in android suspend mode.
	 * Move to JAVA layer by using AlarmManager to trigger timeout.
	 */
    /* Google original 
...
*/
	for (i = 0, l = adapter->devices; l; l = l->next) {
		struct btd_device *dev = l->data;
		if (device_is_paired(dev)) {
			devices[i++] = (char *) device_get_path(dev);
		}
	}
//--- BRCM 
...
//+++ BRCM
	str2ba(address,&dest_bdaddr);
	read_type=read_device_type(&adapter->bdaddr, &dest_bdaddr);
	DBG("adapter_create_device type from file=%d",read_type);
	if ( (read_type == DEVICE_TYPE_UNKNOWN) || (read_type > DEVICE_TYPE_DUALMODE ))
	{
		read_type = DEVICE_TYPE_BREDR;
	}
	if(type!=read_type)
	{
		DBG("Use device type from file. read_type=%d",read_type);
		type=read_type;
	}
						...
						gboolean remove_storage)
{
	...
	if (agent && device_is_authorizing(device))
		agent_cancel(agent);
	DBG("adapter_remove_device type=%d",device_get_type(device));
...
static DBusMessage *adapter_load_input_device_info(DBusConnection *conn,
						DBusMessage *msg, void *data)
{
	...
	info("%s: src = %s, filename = [%s]", __FUNCTION__, srcaddr, filename);
	...
	for (count = 0; count < dev_count; count++) {
		...
		memcpy(dstaddr, &hid_list[count * 18], 17);
		dstaddr[17] = '\0';
		str2ba(dstaddr, &dst);

		if (read_link_key(&adapter->bdaddr, &dst, NULL, NULL) == 0) {
			int      i;
			uint16_t attr_mask;
			uint8_t  sub_class;
			uint8_t  app_id;
			...
			/* Device paired. Add the HID descriptor information to the stack. */
			hid_info[0] = '\0';
			read_hid_info(srcaddr, dstaddr, hid_info);
			// info("%s: addr = %s, hid_info = [%s]", __FUNCTION__, dstaddr, hid_info);

			method.hh_hid_info.hdr.id = DTUN_METHOD_HH_HID_INFO;
			str2ba(dstaddr, &method.hh_hid_info.bdaddr);

			p = hid_info;
			method.hh_hid_info.attr_mask = (uint16_t) hex_str_to_int(p, 4);
			p += 5;
			method.hh_hid_info.sub_class = (uint8_t) hex_str_to_int(p, 2);
			p += 3;
			method.hh_hid_info.app_id = (uint8_t) hex_str_to_int(p, 2);
			p += 3;
			method.hh_hid_info.vendor_id = (uint16_t) hex_str_to_int(p, 4);
			p += 5;
			method.hh_hid_info.product_id = (uint16_t) hex_str_to_int(p, 4);
			p += 5;
			method.hh_hid_info.version = (uint8_t) hex_str_to_int(p, 4);
			p += 5;
			method.hh_hid_info.ctry_code = (uint8_t) hex_str_to_int(p, 2);
			p += 3;
			method.hh_hid_info.dl_len = (uint16_t) hex_str_to_int(p, 4);
			p += 5;

			method.hh_open.hdr.len = sizeof(tDTUN_METHOD_HH_HID_INFO) - sizeof(tDTUN_HDR)
				- DTUN_HH_MAX_DSC_LEN + method.hh_hid_info.dl_len;

			for (i = 0; i < method.hh_hid_info.dl_len; i++) {
				method.hh_hid_info.dsc_list[i] = (uint8_t) hex_str_to_int(p, 2);
				p += 2;
			}
			...
			/* The device is in the hidinfo file but not bonded. This usually shall not
			 * happen. Well, remove it from the hidinfo file anyway as a sanity now.
			 */
			error("%s: Oops, device %s is in hidinfo but not bonded, remove it now...",
						...
						const gchar *address, int *err)
{
	...
	if(!dev)
	{
		DBG("create_device_internal not found search");
	}
	if (dev && dev->le)
	{
		DBG("create_device_internal type=le le=%d",dev->le);
		type = DEVICE_TYPE_LE;
	}
	else
	{
		DBG("create_device_internal type=edr");
...
{
	const gchar *dev_path = device_get_path(device);
//+++ BRCM_LOCAL : check null	 CASE#604503
    if ( path == NULL)
    {
        DBG("device_path_cmp : path is null");
        return -1;
    }
    if (dev_path == NULL)
    {
        DBG("device_path_cmp : dev_path is null");
        return -1;
    }	
//--- BRCM_LOCAL : check null	 CASE#604503
...
//+++ BRCM_LOCAL CSP#603844 (K5 or Audio 2.0 carkit issue)
/* org
...
//--- BRCM_LOCAL
	...
	if (!device_is_connected(device)) {
		DBG("remove_device type=%d",device_get_type(device));
		adapter_remove_device(conn, adapter, device, TRUE);
		return dbus_message_new_method_return(msg);
	}
...
//+++ BRCM
static int add_a2dp_source_record(struct btd_adapter *adapter)
{
	sdp_list_t *svclass_id, *pfseq, *apseq, *root;
	...
	uint16_t avdtp_ver = 0x0102; // TODO: Check this
	int ret = 0;
...
//+++ BRCM
			case AUDIO_SOURCE_SVCLASS_ID:
				ret = add_a2dp_source_record(adapter);
				break;
			default:
				break;
//--- BRCM
...
//+++ BRCM
#ifdef BT_ALT_STACK
	{ "LoadInputDeviceInfo",	"",	"",	adapter_load_input_device_info },
#endif
//--- BRCM
...
#ifdef BT_ALT_STACK
	uint8_t device_type;
	gboolean is_paired_device = FALSE;
	// If the device type is ble then create a ble device with all the link keys
	dtun_add_ble_device_do_append(key, value, &device_type, &is_paired_device);
	if (!is_paired_device && (device_type == DEVICE_TYPE_LE)) {
		debug("create_stored_device_from_profiles is_paired_device=%d",is_paired_device);
		return;
	}
#endif

    device = device_create(connection, adapter, key, device_type);
//--- BRCM
	if (!device)
		return;
...
//+++ BRCM
#ifdef BT_ALT_STACK
	dtun_add_devices_do_append(key, value);
#endif
//--- BRCM
...
//+++ BRCM
static void create_stored_device_from_blelinkkeys(char *key, char *value,
							void *user_data)
{
	struct adapter_keys *keys = user_data;
	struct btd_adapter *adapter = keys->adapter;
	...
	dtun_add_ble_device_do_append(addr, value, &device_type, &is_paired_device);

	info = get_key_info(addr, value);
	if (info)
		keys->keys = g_slist_append(keys->keys, info);

	if (g_slist_find_custom(adapter->devices, addr,
					(GCompareFunc) device_address_cmp))
	{
		error("device is already exist return");
		return;
	}

	device = device_create(connection, adapter, addr, device_type);
	if (device) {
		device_set_temporary(device, FALSE);
		adapter->devices = g_slist_append(adapter->devices, device);
	}
...
//+++ BRCM
	...
	textfile_foreach(filename, create_stored_device_from_blelinkkeys, &keys);
...
//+++ BRCM 
	// Do not add devices from the "types" file, since it's
	// now written during inquiry and not just for paired devices. Paired devices
	// should be taken care of by linkkeys and blelinkkeys.
/* Google original 
...
//Although Bluetooth is set visible on a phone, peer device finds it as unknown device(COD error).
//+++ BRCM_LOCAL : cls[1] == 0, cls[1] == 0. Bluez bug
	/* Google original
	...
	*/
//--- BRCM_LOCAL : Broadcom recommands to use btld's default COD.
...
//--- BRCM
	...
	emit_device_found(adapter->path, paddr,
			...
			"Paired", DBUS_TYPE_BOOLEAN, &paired,
//+++ BRCM 
			/* BRCM: temporarily disabled sending UUIDs as part of DeviceFound, causes
			 * race conditionof Settings app attempting connection prior to SDP being done
			 */
/* Google original 
			"UUIDs", DBUS_TYPE_ARRAY, &dev->uuids, uuid_count,
*/
...
//+++ BRCM
	// TBD - Wenbin: During BT initialization, adapter->scan_mode is default to SCAN_DISABLED.
	//     Hence when ops->set_pwoered(FALSE) is called, no SCAN_MODE change event is sent up
	//     to frameworks. Not sure whether there is any better way. Temporarily do below
	//     hacking first.
	/* Google original
...
//+++ BRCM
void adapter_emit_wbs_config(struct btd_adapter *adapter, uint8_t wbs)
{
    uint32_t wbs_config = wbs;
    DBusMessage *signal;

    DBG("wbs config %d", wbs_config);

    signal = dbus_message_new_signal(adapter->path, ADAPTER_INTERFACE, "AgWbsConfig");
    if (!signal) {
        error("Unable to allocate new %s.AgWbsConfig signal", ADAPTER_INTERFACE);
        return;
    }

    dbus_message_append_args(signal, DBUS_TYPE_UINT32, &wbs_config, DBUS_TYPE_INVALID);
    g_dbus_send_message(connection, signal);
}
//--- BRCM
...
gboolean get_service_from_uuid(const char *uuid, char *service)
{
	...
	} else if (strncasecmp(FILE_TRANSFER_UUID, uuid, sizeof(FILE_TRANSFER_UUID)) == 0) {
...
//+++ BRCM
	...
	// per service
#if 1 //LG_BTUI : use GLOBAL_TRUST
	if ((device_is_trusted(device) == TRUE) && strcmp(uuid, A2DP_SINK_UUID) && strcmp(uuid, AVRCP_REMOTE_UUID)) {
...
#else
	{
		...
		info("adapter_authorize: read_trust: service:%s trust_value:%d",
			 service, trusted);
		if (trusted) {
			adapter->auth_idle_id = g_idle_add_full(G_PRIORITY_DEFAULT_IDLE,
													auth_idle_cb, auth,
...
*/
	err = agent_authorize(agent, device, uuid, agent_auth_cb, auth, g_free);
//--- BRCM 
	if (err < 0)
		g_free(auth);
	else
...
/* Google original 
		...
		adapter_ops = ops;
*/
		struct btd_adapter_ops *save = adapter_ops;
