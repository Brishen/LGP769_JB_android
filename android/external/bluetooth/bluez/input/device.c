...
//+++ BRCM
#ifdef BT_ALT_STACK
static DBusMessage *input_device_virtual_unplug(DBusConnection *conn,
						DBusMessage *msg, void *data)
{
	struct input_device *idev = data;
	tDTUN_DEVICE_METHOD method;

	method.hh_bdaddr.hdr.id = DTUN_METHOD_HH_VUP;
	method.hh_bdaddr.hdr.len = sizeof(tDTUN_METHOD_HH_BDADDR) - sizeof(tDTUN_HDR);
	memcpy(&method.hh_bdaddr.bdaddr, &idev->dst, 6);
...
static DBusMessage *input_device_get_report(DBusConnection *conn,
						DBusMessage *msg, void *data)
{
	DBusMessageIter iter;
	uint32_t rpt_type;
	...
	if (dbus_message_iter_get_arg_type(&iter) != DBUS_TYPE_UINT32)
		return btd_error_invalid_args(msg);
	dbus_message_iter_get_basic(&iter, &rpt_id);
	dbus_message_iter_next(&iter);

	if (dbus_message_iter_get_arg_type(&iter) != DBUS_TYPE_UINT32)
		return btd_error_invalid_args(msg);
	dbus_message_iter_get_basic(&iter, &buf_size);

	method.hh_get_rpt.hdr.id = DTUN_METHOD_HH_GET_RPT;
	method.hh_get_rpt.hdr.len = sizeof(tDTUN_METHOD_HH_GET_RPT) - sizeof(tDTUN_HDR);
	memcpy(&method.hh_get_rpt.bdaddr, &idev->dst, 6);
	method.hh_get_rpt.r_type = (uint8_t) rpt_type;
	method.hh_get_rpt.rpt_id = (uint8_t) rpt_id;
	method.hh_get_rpt.buf_size = buf_size;
...
//+++ BRCM
	{ "VirtualUnplug",	"",	"",	input_device_virtual_unplug	},
	{ "GetProtocolMode","",	"",	input_device_get_protocol_mode },
	{ "SetProtocolMode","u","",	input_device_set_protocol_mode },
	{ "GetReport",		"uuu",	"",	input_device_get_report },
	{ "SetReport",		"us",	"",	input_device_set_report },
	{ "SendData",		"s",	"",	input_device_send_data },
//--- BRCM
