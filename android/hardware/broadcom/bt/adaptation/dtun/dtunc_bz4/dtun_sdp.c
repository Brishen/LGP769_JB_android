...
#define debug(format, ...) ALOGI (format, ## __VA_ARGS__)
                        ...
                        DBusMessage *msg, void *data)
{
    ...
    debug("%s: record = %p ", __FUNCTION__, record);

    sender = dbus_message_get_sender(msg);
    
    err = add_xml_record(conn, sender, record);
    if (err < 0)
        return g_dbus_create_error(msg, ERROR_INTERFACE ".Failed", "Failed to add SDP record");
