...
** Function         enableOps
**
** Description      Function to send DTUN enableOps method message
**
*******************************************************************************/
static DBusMessage *enableOps(DBusConnection *conn, DBusMessage *msg, void *data)
{
    debug(__FUNCTION__);

	dtun_client_call_id_only(DTUN_METHOD_OPS_ENABLE);
	return dbus_message_new_method_return(msg);
}
...
** Function         disableOps
**
** Description      Function to send DTUN disableOps method message
**
*******************************************************************************/
static DBusMessage *disableOps (DBusConnection *conn, DBusMessage *msg, void *data)
{
    debug(__FUNCTION__);

    dtun_client_call_id_only(DTUN_METHOD_OPS_DISABLE);
    return dbus_message_new_method_return(msg);
}
...
static GDBusMethodTable dtun_opp_methods[] = {
    ...
    { "EnableOps",          "",   "",   enableOps,         0 },
    { "DisableOps",          "",  "",   disableOps,        0 },
...
void dtun_sig_opc_progress (tDTUN_DEVICE_SIGNAL *p_data)
{
    ...
    int64_t bytes_transferred = (int64_t) p_data->opc_progress.bytes;
    int64_t obj_size = (int64_t) p_data->opc_progress.obj_size;

    if (!g_dbus_emit_signal(
        conn, BTLA_OBEX_PATH,
        BTLA_OBEX_INTERFACE, "OpcProgress",
        DBUS_TYPE_INT64, &bytes_transferred,
...
void dtun_sig_ops_progress (tDTUN_DEVICE_SIGNAL *p_data)
{
    ...
    int64_t bytes_transferred = (int64_t) p_data->ops_progress.bytes;
    int64_t obj_size = (int64_t) p_data->ops_progress.obj_size;
